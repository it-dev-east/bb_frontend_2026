      ...
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            ...
            <TableHead class="font-semibold text-gray-800">Jenis Kelamin</TableHead>
            <TableHead class="font-semibold text-gray-800">Jenis Kantong Darah</TableHead>
            <TableHead class="font-semibold text-gray-800">Durasi Aftap(menit)</TableHead>
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
           ...
           >
             ...
             <TableCell>{{ result.no_kartu_donor }}</TableCell>
             <TableCell>{{ result.nama_donor }}</TableCell>
             <TableCell>{{ result.jenis_kelamin }}</TableCell>
             <TableCell>{{ result.jenis_kantong_darah }}</TableCell>
             <TableCell>{{ result.durasi_aftap }}</TableCell>
            </TableRow>
...
const handleSearch = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/api/reports/aftap_kinerja', { params })
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/reports/aftap_kinerja/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Kinerja_Aftap_${new Date().toISOString().split('T')[0]}.xlsx`
