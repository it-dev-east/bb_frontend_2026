...
<div class="flex gap-4 mx-4">
  <!-- Data Pengiriman Kantong Darah -->
  <Card class="flex-1 p-2 shadow-md border rounded-2xl bg-background">
    <CardHeader class="border-b px-4 py-4">
      <h3 class="text-xl font-semibold text-foreground tracking-tight">Data Pengiriman Kantong Darah</h3>
    </CardHeader>
          ...
          <div class="flex flex-col">
            ...
            <Input
             v-model="form.nomorPengiriman"
              class="mt-1"
              placeholder="Masukkan nomor pengiriman" @keyup.enter="checkNomorPengiriman" @input="onNomorPengirimanChange"
            />
          </div>
        </div>
  ...
  <!-- Data Penerimaan Kantong Darah -->
  <Card class="flex-1 p-2 shadow-md border rounded-2xl bg-background">
    <CardHeader class="border-b px-4 py-4">
      <h3 class="text-xl font-semibold text-foreground tracking-tight">Data Penerimaan Kantong Darah</h3>
    </CardHeader>
...
<!-- Scan Area -->
<Card class="mt-4 mx-4 p-2 shadow-md border rounded-2xl bg-background">
  <CardHeader class="border-b px-4 py-4">
...
<!-- Bottom area: Grid -->
<Card class="mt-4 mx-4 p-4 shadow-md border rounded-2xl bg-background">
  <CardHeader class="border-b px-4 py-4">
    <h3 class="text-xl font-semibold text-foreground tracking-tight">Penerimaan Kantong Darah</h3>
  </CardHeader>
...
<div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <Card class="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
    ...
    <CardContent>
      <!-- Search Input -->
      <div class="flex items-end gap-4 p-4 bg-white rounded-xl shadow-sm">
...
async function buatPenerimaanBaru() {
  ...
  try {
    const { data } = await api.get('/api/penerimaan/next_nomor')
...
async function searchPengiriman() {
  try {
    const { data } = await api.get('/api/pengiriman/search', { params: { query: searchQuery.value, date: selectedDate.value } })
...
async function pilihPengiriman(item) {
  ...
  // Now check penerimaan
  try {
    // const { data: penerimaanData } = await api.get(`/api/penerimaan/by_nomor_pengiriman/${form.nomorPengiriman}`)
    // form.tanggalPenerimaan = new Date(penerimaanData.tanggal_penerimaan).toISOString().slice(0, 16)
    // form.nomorPenerimaan = penerimaanData.nomor_penerimaan
    // form.namaPetugasPenerimaan = penerimaanData.created_by
    const { data: penerimaanRes } = await api.get(`/api/penerimaan/by_nomor_pengiriman/${form.nomorPengiriman}`)
    const penerimaanList = penerimaanRes.data || []

    if (penerimaanList.length > 0) {
      const penerimaan = penerimaanList[0]
      ...
      form.tanggalPenerimaan = new Date().toISOString().slice(0, 16)
      const { data: nextNomorData } = await api.get('/api/penerimaan/next_nomor')
      form.nomorPenerimaan = nextNomorData.nomor_penerimaan
      form.namaPetugasPenerimaan = '' // TODO: get current user
      await api.post('/api/penerimaan', {
        nomor_pengiriman: form.nomorPengiriman,
        tanggal_penerimaan: form.tanggalPenerimaan,
        nomor_penerimaan: form.nomorPenerimaan,
        created_by: form.namaPetugasPenerimaan
      })
    }
  } catch (penerimaanError) {
    // // Create new
    // form.tanggalPenerimaan = new Date().toISOString().slice(0, 16)
    // const { data: nextNomorData } = await api.get('/api/penerimaan/next_nomor')
    // form.nomorPenerimaan = nextNomorData.nomor_penerimaan
    ...
    //   created_by: form.namaPetugasPenerimaan
    // })
    console.error('❌ Error fetching penerimaan:', penerimaanError.response?.status, penerimaanError.response?.data || penerimaanError.message)
  }
  // Fetch details
  await fetchDetails()
}
...
function onNomorPengirimanChange() {
  if (form.nomorPengiriman && form.nomorPenerimaan) {
    // If manually entered, fetch details
    fetchDetails()
  }
}
async function checkNomorPengiriman() {
  if (!form.nomorPengiriman.trim()) return
  try {
    const { data: pengirimanData } = await api.get(`/api/pengiriman_master/${form.nomorPengiriman}`)
    form.tanggalPengiriman = new Date(pengirimanData.tanggal_pengiriman).toISOString().slice(0, 16)
    form.namaPetugasPengiriman = pengirimanData.created_by
    // Now check penerimaan
    try {
      // const { data: penerimaanData } = await api.get(`/api/penerimaan/by_nomor_pengiriman/${form.nomorPengiriman}`)
      // form.tanggalPenerimaan = new Date(penerimaanData.tanggal_penerimaan).toISOString().slice(0, 16)
    ...
    form.tanggalPengiriman = ''
    form.namaPetugasPengiriman = ''
  }
}
...
async function konfirmasiBarcode() {
  ...
  if (!form.nomorPengiriman) {
    // Check scanned barcode in pengiriman_detail and get nomor_pengiriman
    try {
      const { data: pengirimanData } = await api.get(`/api/pengiriman/by_barcode/${form.scanNomorBarcode}`)
      form.nomorPengiriman = pengirimanData.nomor_pengiriman
      form.tanggalPengiriman = new Date(pengirimanData.tanggal_pengiriman).toISOString().slice(0, 16)
      form.namaPetugasPengiriman = pengirimanData.created_by
      console.log('No Pengiriman:' + form.nomorPengiriman)
      // Now check penerimaan exist
      try {
        const { data: penerimaanRes } = await api.get(`/api/penerimaan/by_nomor_pengiriman/${form.nomorPengiriman}`)
        const penerimaanList = penerimaanRes.data || []
        ...
        if (penerimaanList.length > 0) {
          ...
          form.nomorPenerimaan = penerimaan.nomor_penerimaan
          form.namaPetugasPenerimaan = penerimaan.created_by || ''
          //console.log('No Penerimaan:' + form.nomorPenerimaan)
        } else {
          // Generate new nomor penerimaan
          const { data: nextData } = await api.get('/api/penerimaan_kantong/next_nomor')
          form.nomorPenerimaan = nextData.nomor_penerimaan
          form.tanggalPenerimaan = new Date().toISOString().slice(0, 16)
          form.namaPetugasPenerimaan = userid
          //console.log('No Penerimaan:' + form.nomorPenerimaan)
          await api.post('/api/penerimaan_kantong', {
            nomor_penerimaan: form.nomorPenerimaan,
            nomor_pengiriman: form.nomorPengiriman,
            created_by: userid
          })
        ...
        // Do nothing
        //console.log('error No Penerimaan:' + form.nomorPenerimaan)
        console.error('❌ Error fetching penerimaan:', penerimaanError.response?.status, penerimaanError.response?.data || penerimaanError.message)
      }
      // Fetch details
      await fetchDetails()
      ...
      if (item) {
        ...
        try {
          console.log('No Penerimaan:' + form.nomorPenerimaan)
          console.log('No Barcode:' + form.scanNomorBarcode)
          await api.put('/api/penerimaan_detail/update_status', {
            nomor_penerimaan: form.nomorPenerimaan,
        ...
        await api.post('/api/activity_log', {
          ...
          menu: 'Penerimaan Kantong Darah',
          no_kantong: stokData.nomor_kantong,
          lokasi: 'Ruang Pengolahan',
          komponen: ''
        })
      }
    } catch (error) {
...
async function fetchDetails() {
  ...
  try {
    const { data } = await api.get(`/api/pengiriman/${form.nomorPengiriman}/details`)
...
// Watch for nomorPengiriman changes
watch(() => form.nomorPengiriman, (newVal) => {
  if (newVal) {
    currentPage.value = 1
    fetchDetails()
  } else {
