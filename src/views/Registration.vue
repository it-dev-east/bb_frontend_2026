...
onMounted(async () => {
  ...
  const res = await api.get('/api/public/questionnaire')
  data.value = res.data
  ...
  try {
    const consentRes = await api.get('/api/public/questionnaire/question/999')
  ...
  try {
    const cityRes = await api.get('/api/public/config/default_city')
...
async function uploadPatientPhoto() {
  ...
  const res = await api.post('/api/public/patient-photo', formData, {
...
async function searchMasterDonor() {
  ...
  try {
    const res = await api.get(`/api/public/master_donor/by_nik/${regForm.value.ktp}`);
    const data = res.data;
    // populate regForm
...
async function submitRegistration() {
  ...
  try {
    console.log('Sending POST to /api/public/patients')
    const payload = {
      ...regForm.value,
      full_name: (regForm.value.full_name || "").trim().toUpperCase(),
    }

    const res = await api.post('/api/public/patients', payload)
...
async function submitQuestionnaire() {
  ...
  try {
    await api.post('/api/public/questionnaire/submit', payload)
...
async function printModal() {
  ...
  try {
    ...
    const response = await api.post('/api/public/generate_donor_registration_pdf', pdfData, {
      responseType: 'blob'
    });
