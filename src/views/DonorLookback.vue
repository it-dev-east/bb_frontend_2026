<template>
  ...
  <Card class="m-4 p-2 shadow-md border rounded-2xl bg-background">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
      ...
      <!-- 🔸 RadioGroup Section (One Row) -->
      <div class="flex flex-col w-full lg:w-auto">
        <Label class="text-sm font-medium text-gray-700 mb-2">Cari Berdasarkan</Label>
        <RadioGroup
      ...
      <div class="flex flex-col w-full lg:w-1/2">
        <Label class="text-sm font-medium text-gray-700 mb-2">Masukkan Pencarian</Label>
        <div class="flex gap-2">
          ...
          />
          <Button @click="handleSearch" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">  <Search class="w-5 h-5" /> Cari</Button>
      ...
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            ...
            <TableHead class="font-semibold text-gray-800">Menu</TableHead>
            <TableHead class="font-semibold text-gray-800">Note</TableHead>
            <TableHead class="font-semibold text-gray-800">User ID</TableHead>
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
           ...
           >
             <TableCell>{{ formatDate(result.datetime) }}</TableCell>
             <TableCell>{{ result.regno }}</TableCell>
             <TableCell>{{ result.pid }}</TableCell>
             <TableCell>{{ result.barcodeno }}</TableCell>
             <TableCell>{{ result.menu }}</TableCell>
  ...
  import { RefreshCcw, Plus, Search, Unlock, Barcode, TextSearch } from "lucide-vue-next"
  //shadcnvue
...
const handleSearch = async () => {
  ...
  console.log("Searching by:", selectedOption.value, "Value:", searchQuery.value)
  try {
    const params = {
      search_type: selectedOption.value,
      search_value: searchQuery.value
    }
    const { data } = await api.get('/api/activity_log/search', { params })
