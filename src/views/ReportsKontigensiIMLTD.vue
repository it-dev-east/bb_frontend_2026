    ...
    <CardHeader class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b px-4 py-4">
      <div>
        <h3 class="text-xl font-semibold text-foreground tracking-tight">
          Laporan Kontigensi Uji Saring IMLTD
        </h3>
      </div>
    </CardHeader>
    ...
    <CardContent class="p-4">
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            <TableHead class="font-semibold text-gray-800">Parameter IMLTD</TableHead>
            <TableHead class="font-semibold text-gray-800">Jumlah Unit Masuk</TableHead>
            <TableHead class="font-semibold text-gray-800">Jumlah Reaktif</TableHead>
            <TableHead class="font-semibold text-gray-800">% Reaktif</TableHead>
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
           ...
           >
             <TableCell>{{ result.parameter_imltd }}</TableCell>
             <TableCell>{{ result.jumlah_unit_masuk }}</TableCell>
             <TableCell>{{ result.jumlah_reaktif}}</TableCell>
             <TableCell>{{ result.persen_reaktif }}</TableCell>
            </TableRow>
...
const handleSearch = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/api/reports/imltd/contingency', { params })
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/reports/imltd/contingency/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Kontigensi_IMLTD_${new Date().toISOString().split('T')[0]}.xlsx`
