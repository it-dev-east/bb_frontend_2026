      ...
      <Table>
        <TableHeader class="bg-gray-100">
           <TableRow>
              ...
              <TableHead class="font-semibold text-gray-800">Total</TableHead>
        ...
        <TableBody>
           ...
           >
              <TableCell>{{ result.donation_date }}</TableCell>
...
const handleSearch = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/api/reports/patients/donation_count', { params })
  ...
  try {
    ...
    const response = await api.get(
      '/api/reports/patients/donation_count/export_excel',
    ...
    link.download = `Laporan_Jumlah_Pendonor_${new Date().toISOString().split('T')[0]}.xlsx`
