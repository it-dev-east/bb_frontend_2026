<template>
  ...
  <div class="flex flex-col gap-3">
   <Label class="text-sm font-medium text-gray-700">Cari Berdasarkan</Label>
    ...
    <!-- Wrap selects in a flex row -->
    <div class="flex flex-wrap items-end gap-4">
  <!-- Golongan Darah -->
  <div class="flex flex-col">
    <label class="text-sm font-medium text-gray-700 mb-1">Golongan Darah</label>
    <Select v-model="selectedGolonganDarah" class="w-40">
      <SelectTrigger>
  ...
  <!-- Rhesus -->
  <div class="flex flex-col">
    <label class="text-sm font-medium text-gray-700 mb-1">Rhesus</label>
    <Select v-model="selectedRhesus" class="w-40">
      <SelectTrigger>
        <SelectValue placeholder="Pilih" />
      </SelectTrigger>
      <SelectContent>
  ...
  <!-- Komponen -->
  <div class="flex flex-col">
    <label class="text-sm font-medium text-gray-700 mb-1">Komponen</label>
    <Select v-model="selectedKomponen" class="w-56">
      <SelectTrigger>
        <SelectValue placeholder="Pilih" />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="all">All</SelectItem>
        <SelectItem value="10">(PRC) Packed Red Cells</SelectItem>
        <SelectItem value="12">(TC) Thrombocyte Concentrate</SelectItem>
        <SelectItem value="13">(FFP) Fresh Frozen Plasma</SelectItem>
        <SelectItem value="14">(CRYO) Cryoprecipitate</SelectItem>
        <SelectItem value="21">(LEU) Leukosit Apheresis</SelectItem>
        <SelectItem value="22">(TCA) Trombosit Apheresis</SelectItem>
        <SelectItem value="23">(PLA) Plasma Apheresis</SelectItem>
        <SelectItem value="24">(RCA) Eritrosit Apheresis</SelectItem>
        <SelectItem value="50">(WB) Whole Blood</SelectItem>
      </SelectContent>
    </Select>
  </div>
  ...
  <div class="flex flex-col">
    ...
    <Select v-model="selectedStatus" class="w-44">
      <SelectTrigger>
        <SelectValue placeholder="Pilih" />
      </SelectTrigger>
      <SelectContent>
        ...
        <SelectItem value="Siap Pakai">Siap Pakai</SelectItem>
        <SelectItem value="Sudah Terpakai">Sudah Terpakai</SelectItem>
        <SelectItem value="Batal">Batal</SelectItem>
       <!-- <SelectItem value="Selesai Diolah">Selesai Diolah</SelectItem>  -->
      </SelectContent>
    </Select>
  </div>
</div>
      ...
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            ...
            <TableHead class="font-semibold text-gray-800">Komponen</TableHead>
            <TableHead class="font-semibold text-gray-800">Status</TableHead>
            <TableHead class="font-semibold text-gray-800">Jumlah Stok</TableHead>
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
           ...
           >
             ...
             <TableCell>{{ result.komponen }}</TableCell>
             <TableCell>{{ result.status }}</TableCell>
             <TableCell>{{ result.jumlah_stok }}</TableCell>
            </TableRow>
  ...
  // New reactive refs for filters
  const selectedGolonganDarah = ref("all")
  const selectedRhesus = ref("all")
  const selectedKomponen = ref("all")
  const selectedStatus = ref("Siap Pakai")
...
const handleSearch = async () => {
  ...
  try {
    ...
    if (selectedGolonganDarah.value && selectedGolonganDarah.value !== 'all') {
      params.golongan_darah = selectedGolonganDarah.value
    }
    if (selectedRhesus.value && selectedRhesus.value !== 'all') {
      params.rhesus = selectedRhesus.value
    }
    if (selectedKomponen.value && selectedKomponen.value !== 'all') {
      params.komponen = selectedKomponen.value
    }
    ...
    const { data } = await api.get('/api/stok_darah/summary', { params })
  ...
  function clearSearch() {
    ...
    searchResults.value = []
    currentPage.value = 1
    selectedGolonganDarah.value = "all"
    selectedRhesus.value = "all"
    selectedKomponen.value = "all"
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/stok_darah/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Kartu_Stok_${new Date().toISOString().split('T')[0]}.xlsx`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      window.URL.revokeObjectURL(downloadUrl)
    } catch (error) {
      // kalau server kirim JSON/text error, baca dulu
