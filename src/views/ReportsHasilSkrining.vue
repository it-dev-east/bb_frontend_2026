      ...
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            ...
            <TableHead class="font-semibold text-gray-800">Hasil Sifilis</TableHead>
            <TableHead class="font-semibold text-gray-800">Hasil HBsAg</TableHead>
            <TableHead class="font-semibold text-gray-800">Hasil HCV</TableHead>
            <TableHead class="font-semibold text-gray-800">Hasil HIV</TableHead>
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
           ...
           >
             ...
             <TableCell>{{ result.sifilis_kesimpulan }}</TableCell>
             <TableCell>{{ result.hbsag_kesimpulan }}</TableCell>
             <TableCell>{{ result.hcv_kesimpulan }}</TableCell>
             <TableCell>{{ result.hiv_kesimpulan }}</TableCell>
            </TableRow>
...
const handleSearch = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/api/hasil_skrining/search', { params })
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/hasil_skrining/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Hasil_Skrining_${new Date().toISOString().split('T')[0]}.xlsx`
