    ...
    <CardHeader class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b px-4 py-4">
      <div>
        <h3 class="text-xl font-semibold text-foreground tracking-tight">
          Laporan Penerimaan Kantong Darah di Ruang Pengolahan
        </h3>
      </div>
    </CardHeader>
...
const handleSearch = async () => {
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/log_cekal/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Donor_Ditolak_${new Date().toISOString().split('T')[0]}.xlsx`
