      ...
      <Table>
        <TableHeader class="bg-gray-100">
          <TableRow>
            ...
            <TableHead class="font-semibold text-gray-800">No Pengiriman</TableHead>
        ...
        <TableBody>
           ...
           >
             <TableCell>{{ formatDate(result.tanggal_pengiriman) }}</TableCell>
             <TableCell>{{ result.nomor_pengiriman }}</TableCell>
             <TableCell>{{ result.scan_nomor_barcode }}</TableCell>
             <TableCell>{{ result.nama_petugas }}</TableCell>
            </TableRow>
...
const handleSearch = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/api/reports/pengiriman/search', { params })
  ...
  const exportExcel = async () => {
    try {
      ...
      const response = await api.get('/api/reports/pengiriman/export_excel', { params, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      const downloadUrl = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = downloadUrl
      link.download = `Laporan_Pengiriman_Darah_${new Date().toISOString().split('T')[0]}.xlsx`
